...
import com.wzy.helptravel.bean.Friend;
...
public class UserModel extends BaseModel {
     ...
     * @param listener
     */
    public void register(String username, String password, String phonenum, final LogInListener listener) {
        if (TextUtils.isEmpty(username)) {
            listener.internalDone(new BmobException(CODE_NULL, "请填写用户名"));
            return;
        }
...
//        }
     ...
     * @param listener
     */
    public void updateUserInfo(final User user, final UpdateCacheListener listener) {

        if (user.getAvatar() != null) {
            try {
                Uri avatarUri = Uri.parse(user.getAvatar());
                final BmobFile avatarFile = new BmobFile(new File(BitmapTools.getRealFilePath(getContext(), avatarUri)));
                avatarFile.uploadblock(getContext(), new UploadFileListener() {
                    @Override
                    public void onSuccess() {
                        user.setAvatar(avatarFile.getUrl());
